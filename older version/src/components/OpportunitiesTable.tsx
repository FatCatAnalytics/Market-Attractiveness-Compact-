...
export function OpportunitiesTable({ globalFilters, bucketAssignments, mapData = [] }: OpportunitiesTableProps) {
  const [opportunities, setOpportunities] = useState<OpportunityData[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [showCompetitiveLandscape, setShowCompetitiveLandscape] = useState(false); // Hidden by default
  ...
  const providerAggregates = useMemo(() => {
    ...
    filteredOpportunities.forEach(opp => {
      ...
      if (opp.Weighted_Average_Score !== null && opp.Weighted_Average_Score !== undefined && !isNaN(opp.Weighted_Average_Score)) {
        aggregate.satisfactionScores.push(opp.Weighted_Average_Score);
      }
    });
              ...
              <Table className="w-full table-auto">
                ...
                <TableBody>
                  {providerAggregates.map((agg, idx) => {
                    ...
                    return (
                      <TableRow key={agg.provider}>
                        <TableCell className="font-medium px-2 text-xs">{agg.provider}</TableCell>
