...
export function MSACompetitiveTable({ opportunities, limit = 50, hideMarketSize = false, allOpportunities, selectedProviders, onToggleProviderSelection }: MSACompetitiveTableProps) {
  ...
  const providerSatisfactionScores = useMemo(() => {
    ...
    opportunities.forEach(opp => {
      ...
      // Try alternative key names if the primary one is not found
  ...
  const providerMSAsCounts = useMemo(() => {
    ...
    // Use allOpportunities for national count, fallback to local opportunities
  ...
  const applyNumericFilter = (value: number, filter: NumericFilter): boolean => {
    ...
    switch (filter.operator) {
      ...
      case '<':
        return value < filterValue;
      case '=':
        return Math.abs(value - filterValue) < 0.01; // Allow small tolerance for floats
      default:
        return true;
    }
  };
  ...
  const processedData = useMemo(() => {
    ...
    let filtered = opportunities.filter(opp => {
      ...
      // Market share filter
      const marketShare = parseFloat(String(opp["Market Share"] || 0)) * 100; // Convert to percentage
      ...
      if (satisfactionScore !== undefined) {
        ...
        return false; // Exclude if filter is active but no score
      }
