...
import { getRegionFromStateCodes } from "../utils/stateToRegionMapping";
  ...
  return (
    <div className="space-y-4">
      ...
      {(selectedProviders.size > 0 || selectedMSA !== "all") && (() => {
        ...
        // Use marketData (all providers) instead of data (only ranked) to include JPMorgan, Wells Fargo, etc.
        const providersToShow = selectedProviders.size > 0 
          ? selectedProviders 
          : new Set(filteredMarketData.map(opp => opp.Provider));
        
        // Filter marketData for the selected providers (includes all providers, not just ranked ones)
        const filteredMarketDataWithoutMSA = marketData.filter(item => {
          const matchesGlobalFilters = globalFilteredMSAs.has(item.MSA);
          const matchesProvider = selectedProviders.size === 0 || selectedProviders.has(item.Provider);
          const matchesCategory = selectedCategory === "all" || item.Opportunity_Category === selectedCategory;
          const matchesAttractiveness = selectedAttractiveness === "all" || item.Attractiveness_Category === selectedAttractiveness;
        ...
        const providerAggregates = Array.from(providersToShow).map(provider => {
          ...
          // Calculate regional data for this provider
          const regionalOpps = providerOpps.map(opp => ({
            msa: opp.MSA,
            region: getRegionFromStateCodes(opp.MSA),
            marketShare: parseFloat(String(opp["Market Share"] || 0)) * 100,
            category: opp.Attractiveness_Category || 'Unknown'
          }));
          
          const regionGroupsMap = new Map<string, typeof regionalOpps>();
          regionalOpps.forEach(opp => {
            if (!regionGroupsMap.has(opp.region)) {
              regionGroupsMap.set(opp.region, []);
            }
            regionGroupsMap.get(opp.region)!.push(opp);
          });
          
          const regionOrder = ['Southeast', 'Southwest', 'Mideast', 'Great Lakes', 'Far West', 'Plains', 'New England', 'Rocky Mountain', 'Other'];
          const regionalData = Array.from(regionGroupsMap.entries())
            .map(([region, msas]) => ({
              region,
              totalMSAs: msas.length,
              topMSAs: msas.sort((a, b) => b.marketShare - a.marketShare).slice(0, 3)
            }))
                                  ...
                                  <div className="p-3 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div>
                                        ...
                                        <p className="text-[10px] text-muted-foreground">{agg.regionCount} regions · {agg.msaCount} MSAs</p>
                                      </div>
                                    </div>
                                  </div>
                                    ...
                                    {agg.regionalData.map((regionData, rIdx) => (
                                      <div key={rIdx} className="bg-white dark:bg-slate-800/50 rounded-md p-2 border border-slate-100 dark:border-slate-700">
                                        ...
                                        <div className="space-y-1 pl-4">
                                          {regionData.topMSAs.map((msa, mIdx) => {
                                            const maxShare = regionData.topMSAs[0]?.marketShare || 1;
                                            const cleanMsaName = msa.msa.replace(/^[A-Z]{2}(-[A-Z]{2})*-/, '');
                                            return (
                                              <div key={mIdx} className="flex items-center gap-1.5">
                                                <span className="text-[9px] text-slate-400 w-2">{mIdx + 1}.</span>
                                                <div className="flex-1 min-w-0">
                                                  <div className="flex items-center justify-between">
                                                    <span className="text-[10px] text-slate-600 dark:text-slate-400 truncate" title={msa.msa}>
                                                      {cleanMsaName}
                                                    </span>
                                                    <span className="text-[10px] font-medium ml-1">{msa.marketShare.toFixed(1)}%</span>
                                                  </div>
                                                  <div className="h-1 bg-slate-200 dark:bg-slate-600 rounded-full overflow-hidden mt-0.5">
                                                    <div 
                                                      className="h-full rounded-full"
                                                      style={{ 
                                                        width: `${(msa.marketShare / maxShare) * 100}%`,
                                                        backgroundColor: regionColors[regionData.region] || '#9ca3af'
                                                      }}
                                                    />
                                                  </div>
                                                </div>
